...
import { CalendarDays, Menu, ShoppingBag, X } from "lucide-react";
import { useCart } from "@/context/CartContext";
import { CIUDADES_ENTREGA, IMAGENES, NAV_LINKS } from "@/data/mockData";
import CitySelect from "./CitySelect";

export default function Header() {
  const { itemCount, openCart, ciudad } = useCart();
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-forest/80 shadow-lg shadow-black/10 backdrop-blur-xl">
      {/* ── Barra de día de entrega ────────────────────────────────────── */}
      <div
        aria-live="polite"
        className="flex h-8 items-center justify-center gap-2 bg-organic/80 px-4 text-xs font-semibold text-white/90 backdrop-blur"
      >
        <CalendarDays aria-hidden="true" className="h-3.5 w-3.5 shrink-0" />
        <span>{entregaInfo.banner}</span>
      </div>

      {/* ── Barra principal ────────────────────────────────────────────── */}
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 md:h-[4.5rem] lg:px-8">
        ...
        <div className="ml-auto flex min-w-0 items-center gap-2 sm:gap-3 lg:ml-0">
          <CitySelect id="city-header" className="min-w-0 max-w-[11rem] sm:max-w-[14rem]" />
